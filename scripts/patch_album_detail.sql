-- Patch: add difficulty, match_rules, target_slots to album detail return
  ...
  ELSE
    ...
    owned AS (
      ...
      WHERE p.owner_id = v_user_id
        ...
        AND (v_album.match_rules->'required_tags' IS NULL OR (p.visual_tags @> (v_album.match_rules->'required_tags')))
        AND (v_album.match_rules->'any_tags' IS NULL OR EXISTS (SELECT 1 FROM jsonb_array_elements_text(v_album.match_rules->'any_tags') tag WHERE p.visual_tags ? tag))
    ...
    numbered_owned AS (
      SELECT rn AS slot_order,
        jsonb_build_object('slot_label', COALESCE(city, category, 'Postal'), 'slot_order', rn, 'postcard_id', id, 'illustration_url', illustration_url, 'city', city, 'country', country, 'category', category, 'is_owned', true, 'is_claimed', true, 'is_hint', false) AS slot_data
      FROM owned
    ),
    hint_slot AS (
      SELECT (SELECT COALESCE(max(rn), 0) FROM owned) + 1 AS slot_order,
        jsonb_build_object('slot_label', COALESCE(city, category, '¿Qué será?'), 'slot_order', (SELECT COALESCE(max(rn), 0) FROM owned) + 1, 'postcard_id', id, 'illustration_url', illustration_url, 'city', city, 'country', country, 'category', category, 'is_owned', false, 'is_claimed', false, 'is_hint', true) AS slot_data
      FROM hint LIMIT 1
    ),
    empty_slots AS (
      SELECT gs AS slot_order,
        jsonb_build_object('slot_label', '???', 'slot_order', gs, 'postcard_id', NULL, 'illustration_url', NULL, 'city', NULL, 'country', NULL, 'category', NULL, 'is_owned', false, 'is_claimed', false, 'is_hint', false) AS slot_data
      FROM generate_series((SELECT COALESCE(max(rn), 0) FROM owned) + 1 + (SELECT count(*) FROM hint), v_album.target_slots) AS gs
    ),
    all_slots AS (
      SELECT slot_order, slot_data FROM numbered_owned
      UNION ALL SELECT slot_order, slot_data FROM hint_slot
      UNION ALL SELECT slot_order, slot_data FROM empty_slots
    )
    SELECT COALESCE(jsonb_agg(slot_data ORDER BY slot_order), '[]'::jsonb)
    INTO v_slots FROM all_slots;
  END IF;
