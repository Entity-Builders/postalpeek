-- Fix: postalpeek_get_album_detail must use dynamic (match_rules) path when
-- the album has match_rules defined, even if physical slots exist.
-- Root cause: some albums have both match_rules AND postalpeek_album_slots 
-- (template/placeholder slots with owner_id = NULL). The old logic checked
-- "has slots?" first, causing slot-based evaluation where is_owned always 
-- returned false because the slot postcards have no owner.
-- Fix: if match_rules is non-empty → always use dynamic path.
...
BEGIN
  ...
  -- Dynamic = has non-empty match_rules (takes priority over physical slots)
  v_is_dynamic := (
    v_album.match_rules IS NOT NULL 
    AND v_album.match_rules != '{}'::jsonb
    AND v_album.match_rules != 'null'::jsonb
  );

  -- Slot-based only if NO match_rules
  ...
  IF v_has_slots THEN
    -- SLOT-BASED: existing behavior (only for albums without match_rules)
