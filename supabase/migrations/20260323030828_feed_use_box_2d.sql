-- Relax the feed filter: Check for either box_2d or bbox so more postcards are picked up.
-- The illustration_tags + bbox/box_2d requirement stays (game mode needs them).
...
BEGIN
  ...
  IF p_albums_only THEN
    ...
    WHERE
      ...
      AND ((p.illustration_tags->0->'bbox') IS NOT NULL OR (p.illustration_tags->0->'box_2d') IS NOT NULL)
  ...
  ELSE
    ...
    WHERE
      ...
      AND ((illustration_tags->0->'bbox') IS NOT NULL OR (illustration_tags->0->'box_2d') IS NOT NULL)
    ORDER BY random()
    LIMIT v_remaining_limit;
  END IF;
END;
