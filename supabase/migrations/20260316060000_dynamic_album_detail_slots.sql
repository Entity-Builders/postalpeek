-- Dynamic album detail slots: target_slots + rewritten album detail RPC

-- 1. Add target_slots to albums (how many postcards to collect for dynamic album completion)
ALTER TABLE postalpeek_albums
  ADD COLUMN IF NOT EXISTS target_slots INT DEFAULT 8;

-- 2. Rewrite album detail RPC to support both slot-based and dynamic albums
...
BEGIN
  ...
  ELSE
    ...
    WITH
    -- All postcards matching this album's rules that the user OWNS
    owned AS (
    ...
    -- One random matching postcard NOT owned by user (for the hint slot)
    hint AS (
      SELECT p.id, p.illustration_url, p.city, p.country, p.category
      FROM postalpeek_postcards p
    ...
    -- Build slots: owned first, then hint, then empty placeholders
    numbered_owned AS (
