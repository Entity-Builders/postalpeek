-- Fix spotlight search RPC: both visual_tags and illustration_tags are jsonb columns.
-- The && operator cannot be used directly between jsonb and text[].
-- We use ARRAY(SELECT jsonb_array_elements_text(...)) to cast to text[] first,
-- then && is safe. This supersedes 20260318184600 with the same correct approach.
