-- Fix: Show blurred preview images for empty/mystery slots in dynamic albums
--
-- Context: Dynamic album empty_slots previously returned illustration_url = NULL,
-- meaning the frontend could only show a blank placeholder.
-- This migration makes empty slots pick a random matching postcard from the pool
-- (that the user doesn't own) to use as a blurred teaser image.
--
-- This improves the "mystery" UX: users can see silhouettes of cards they need.
...
BEGIN
  ...
  ELSE
    ...
    -- Pool of matching postcards (not owned) to use as blurred previews for empty slots
    preview_pool AS (
      SELECT p.id, p.illustration_url, p.city, p.country, p.category,
             row_number() OVER (ORDER BY random()) AS rn
      FROM postalpeek_postcards p
      WHERE p.illustration_url IS NOT NULL
        AND p.owner_id IS DISTINCT FROM v_user_id
        AND p.id NOT IN (SELECT id FROM hint)
        AND (v_album.match_rules->>'country' IS NULL OR v_album.match_rules->>'country' = p.country)
        AND (v_album.match_rules->>'city' IS NULL OR v_album.match_rules->>'city' = p.city)
        AND (
          v_album.match_rules->'required_tags' IS NULL
    ...
    -- Empty slots: show a random matching postcard as a blurred teaser
    empty_slots AS (
      SELECT gs AS slot_order,
        jsonb_build_object(
          ...
          'slot_order', gs,
          'postcard_id', NULL,
          -- Pick a random preview image from the pool (cycling by modulo if pool is small)
          'illustration_url', (
            SELECT illustration_url FROM preview_pool
            WHERE rn = ((gs - (SELECT COALESCE(max(s_rn), 0) FROM (SELECT max(rn) AS s_rn FROM owned) _) - 1) % GREATEST((SELECT count(*) FROM preview_pool), 1)) + 1
          ),
