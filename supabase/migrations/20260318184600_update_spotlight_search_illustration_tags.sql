-- Same migration for PostalPeek app's cloud project migrations
-- Mirror of eb-infra/supabase/migrations/20260318184600_update_spotlight_search_illustration_tags.sql
...
BEGIN
  ...
  WHERE
    ...
    AND (p_free_text IS NULL OR (
      ...
      EXISTS (
        SELECT 1 FROM jsonb_array_elements_text(COALESCE(p.illustration_tags, '[]'::jsonb)) AS it
