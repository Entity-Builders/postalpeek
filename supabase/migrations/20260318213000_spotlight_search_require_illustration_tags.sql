-- Add p_require_illustration_tags parameter to postalpeek_spotlight_search.
-- When TRUE  → only postcards WITH illustration_tags are matched (strict mode).
-- When FALSE → existing fallback behaviour: prefers illustration_tags, falls back to visual_tags.
  ...
  WHERE
    ...
    AND (array_length(p_tags, 1) IS NULL OR (
      ...
      ARRAY(SELECT jsonb_array_elements_text(
        CASE
          WHEN p_require_illustration_tags THEN
            -- Strict: only illustration_tags; rows without them were already filtered above.
            COALESCE(p.illustration_tags, '[]'::jsonb)
          ELSE
            -- Fallback: illustration_tags if present, else visual_tags.
            CASE
