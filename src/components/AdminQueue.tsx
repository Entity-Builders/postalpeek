  ...
  return (
    <div className="flex items-center justify-between mb-4">
      <div className="flex items-center gap-2.5">
        ...
        {badge != null && (
          <span className="px-2 py-0.5 rounded-full text-[10px] font-mono font-medium text-white/50"
...
function StatusChip({ status }: { status: CronLogEntry['status'] }) {
  const config = {
    success: { icon: <CheckCircle className="w-3 h-3" />, label: 'success', color: 'text-emerald-400 bg-emerald-950/60 border-emerald-700/30' },
    error:   { icon: <XCircle className="w-3 h-3" />,     label: 'error',   color: 'text-red-400 bg-red-950/60 border-red-700/30' },
    skipped: { icon: <SkipForward className="w-3 h-3" />, label: 'skipped', color: 'text-amber-400 bg-amber-950/60 border-amber-700/30' },
  };
...
export function AdminQueue() {
  ...
  // ── Cron status ──
  const [cronStatus, setCronStatus] = useState<{ status: ActionStatus; message: string }>({ status: 'idle', message: '' });
  ...
  }, [fetchPendingSlots, fetchCronLog]);

  // Stop auto-run on unmount
  useEffect(() => () => { autoRunRef.current = false; }, []);
  ...
  const runCron = useCallback(async (slotId?: string) => {
    setCronStatus({
      status: 'loading',
      message: slotId ? `Processing slot ${slotId.slice(0, 8)}…` : 'Running cron walker…',
    });
    try {
      ...
      setCronStatus({ status: 'error', message: err instanceof Error ? err.message : String(err) });
  ...
  const startAutoRun = useCallback(async () => {
    ...
    while (autoRunRef.current) {
      try {
        ...
        if (remaining === 0) {
          // Queue drained!
          setCronStatus({ status: 'success', message: `🎉 Queue drained! Processed ${count} slot${count !== 1 ? 's' : ''}.` });
          break;
        }
  ...
  return (
    <div className="max-w-3xl space-y-10">
      ...
      {/* ── 1. Cron Controls ── */}
      <section>
        <SectionHeader icon={<Zap className="w-4 h-4" />} title="Cron Controls" />
        <div
          className="rounded-2xl p-5 space-y-4"
          style={{
            background: autoRunning ? 'rgba(16,185,129,0.06)' : 'rgba(99,102,241,0.05)',
        ...
        >
          ...
          <div className="flex items-start gap-3">
            <div className="flex-1">
              <p className="text-white/70 text-sm font-medium mb-1">Walker Cron</p>
              <p className="text-white/30 text-xs">
                Triggers <code className="text-indigo-300/70 font-mono text-[10px]">postalpeek-cron-walker?force=true</code> as <span className="text-violet-300">admin</span>.
                Usa <span className="text-emerald-300/80">Auto-Run</span> para procesar todos los slots pendientes en bucle.
              </p>
            </div>
            ...
            <div className="flex gap-2 shrink-0">
              ...
              <button
                ...
                style={{ background: 'linear-gradient(135deg, rgba(99,102,241,0.7), rgba(139,92,246,0.7))', border: '1px solid rgba(99,102,241,0.3)' }}
              >
              ...
              {autoRunning ? (
                <button
                  ...
                  style={{ background: 'linear-gradient(135deg, rgba(239,68,68,0.6), rgba(220,38,38,0.6))', border: '1px solid rgba(239,68,68,0.3)' }}
                >
                ...
                <button
                  ...
                  disabled={cronStatus.status === 'loading' || pendingSlots.length === 0}
                  title={pendingSlots.length === 0 ? 'No hay slots pendientes' : 'Procesar toda la cola automáticamente'}
                  className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold text-white transition-all duration-200 disabled:opacity-30 disabled:cursor-not-allowed hover:brightness-110"
                  style={{ background: 'linear-gradient(135deg, rgba(16,185,129,0.6), rgba(5,150,105,0.6))', border: '1px solid rgba(16,185,129,0.3)' }}
                >
                  <Repeat className="w-4 h-4" />
                  Auto-Run
                  {pendingSlots.length > 0 && (
                    <span className="ml-1 px-1.5 py-0.5 rounded-full text-[10px] font-mono bg-emerald-900/60 text-emerald-300">
                      {pendingSlots.length}
                    </span>
                  )}
                </button>
          ...
          {slotsLoading && pendingSlots.length === 0 ? (
            ...
            <table className="w-full text-left text-sm">
              <thead style={{ background: 'rgba(255,255,255,0.04)' }}>
                <tr>
                  <th className="px-4 py-2.5 text-xs text-white/40 font-medium uppercase tracking-wider">Order</th>
                  <th className="px-4 py-2.5 text-xs text-white/40 font-medium uppercase tracking-wider">Slot</th>
                  <th className="px-4 py-2.5 text-xs text-white/40 font-medium uppercase tracking-wider">Album</th>
                  <th className="px-4 py-2.5 text-xs text-white/40 font-medium uppercase tracking-wider w-10">Run</th>
                </tr>
              </thead>
              <tbody className="divide-y" style={{ borderColor: 'rgba(255,255,255,0.04)' }}>
                {pendingSlots.map((slot, idx) => (
                  <tr key={slot.id} className="hover:bg-white/4 transition-colors group">
                    <td className="px-4 py-3">
                      {idx === 0
                        ? <span className="text-indigo-400 font-bold text-xs">▶ NEXT</span>
                        : <span className="text-white/35 text-xs font-mono">#{slot.slot_order}</span>}
                    </td>
                    <td className="px-4 py-3 font-medium text-white/80 text-sm">{slot.slot_label}</td>
                    <td className="px-4 py-3 text-white/45 text-xs">{slot.postalpeek_albums?.title || '—'}</td>
                    <td className="px-4 py-3">
                      <button
      ...
      <section>
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      {entry.status === 'error' ? (
                        ...
                        <p className="text-white/30 text-xs italic">No location</p>
                      )}
        ...
        <p className="text-white/20 text-[10px] mt-2 ml-1">Auto-refreshes every 15 seconds</p>
      </section>
    </div>
  );
}
