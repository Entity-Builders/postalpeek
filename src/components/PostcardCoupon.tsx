...
export function PostcardCoupon({ item }: PostcardCouponProps) {
  ...
  const mapQuery = encodeURIComponent(`${item.location_name || ''} ${item.city || ''}`);
  const mapsUrl = `https://www.google.com/maps/search/?api=1&query=${mapQuery}`;

  // Default context message or custom message if it exists in metadata
  const defaultMessage = "Aprovechá este beneficio exclusivo en tu próxima visita.";
  const offerMessage = item.generation_metadata?.offer_message || defaultMessage;
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-[320px] mx-auto bg-white border-2 border-dashed border-stone-300 rounded-lg p-6 flex flex-col items-center shadow-sm">
        ...
        <h2 className="font-serif text-2xl font-bold mb-1 leading-tight" style={{ color: '#1e293b' }}>
          Special Offer
        </h2>
        <h3 className="text-sm font-medium tracking-wide uppercase mb-3" style={{ color: '#78716c' }}>
          at {item.location_name || item.city}
        </h3>

        {/* Content message */}
        <p className="border-t border-b border-stone-100 py-2.5 text-xs italic mb-4 w-full" style={{ color: '#57534e' }}>
          "{offerMessage}"
        </p>
        ...
        {/* Small print */}
        <p className="text-[9px] mt-6 leading-tight max-w-[80%] mx-auto" style={{ color: '#a8a29e' }}>
          Valid only at {item.location_name || 'participating locations'}. Cannot be combined with other offers.
        </p>
      </div>
      
      <p className="absolute bottom-4 text-[10px] font-mono tracking-widest uppercase" style={{ color: '#a8a29e' }}>
