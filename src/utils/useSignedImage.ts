...
import { cdnImage, cdnSrcSet, imageSignerEvents } from './imageUtils';
 ...
 */
export function useSignedImage(url: string | undefined | null, opts: TransformOptions = {}) {
  // Compute the initial URL (might be unsigned if preSignUrls is still running)
  const [signedUrl, setSignedUrl] = useState(() => url ? cdnImage(url, opts) : '');
 ...
 */
export function useSignedSrcSet(url: string | undefined | null, widths: number[]) {
  const [srcSet, setSrcSet] = useState(() => url ? cdnSrcSet(url, widths) : '');
